...
bool LoadBackground()
{
    bool ret = g_background.LoadImageFile("img//bkgn.bmp", g_screen);
    return ret;
}
...
int main(int argc, char* argv[])
{
    ...
    //Make menu game
    int ret_menu = SDLCommonFunc::ShowMenu(g_screen, g_font_MENU, "Play Game", "Exit", "img//MENU.bmp");
    if (ret_menu == 1)
        quit = true;
    ...
    bool ret = g_ground.LoadImageFile("img//ground2.bmp", g_screen);
    if (ret == false)
    {
        return -1;
    }
...
again_label:
    ...
    PlayerObject player;
    ret = player.LoadImg("img//fl_bird.bmp", g_screen);
    ...
    while (!quit)
    {
        ...
        SDL_RenderClear(g_screen); //clear the screen
